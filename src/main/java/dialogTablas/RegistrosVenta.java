...
public class RegistrosVenta extends javax.swing.JDialog {
	
	VentaDelegate del = new VentaDelegate();
	public int idVentaForm;
	public int item;

	public RegistrosVenta(java.awt.Frame parent, boolean modal) {
		super(parent, modal);
		initComponents();
	...
	private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {
		...
		idVentaForm = Integer.parseInt(jTable1.getValueAt(registroSeleccionado, 0).toString());
	}
	...
	private void crearModeloDeTabla() {
		...
		String columnNames[] = { "idVenta", "FechaVenta", "idCliente", "Total"};
		DefaultTableModel modelo = new DefaultTableModel(columnNames, 0);

		for (Venta consola : del.findAll()) {
			Object row[] = new Object[4];
			row[0] = consola.getIdVenta();
			row[1] = consola.getFechaVenta();
			row[2] = consola.getIdCliente().getIdCliente();
			row[3] = consola.getTotal();
